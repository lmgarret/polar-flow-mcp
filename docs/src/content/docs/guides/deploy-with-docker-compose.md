...
[Why single-user?](/polar-flow-mcp/explanation/design-decisions/#single-user-via-env-vars)
for the reasoning.
...
- **Public (Claude.ai web/mobile + Claude Code)** — set `OAUTH_PUBLIC_URL` to
  ...
  proxy. See [Expose the server securely](/polar-flow-mcp/guides/expose-securely/) for the full Caddy +
